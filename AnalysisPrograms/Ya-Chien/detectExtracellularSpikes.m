function [spk,spk_w,t_spk_w,spk_idx,threshold] = detectExtracellularSpikes(data, time, thr, tpre, tpost, tdead, detection_mode)
% DETECTEXTRACELLULARSPIKES Extracts the spikes from extracellular filtered data using the algorithm proposed by Rodrigo Quiroga.
%
% [spk,spk_w,t_spk_w,spk_idx,threshold] = detectExtracellularSpikes(data, time, thr, tpre, tpost, tdead, detection_mode)
...
% LOCATE SPIKE TIMES
for ii = 1:N
    thr = find_spike_threshold(data(ii,:),thr);
    threshold{ii} = thr;
